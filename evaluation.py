...
#             sentence_rep = features['sentence_embedding']
#             model_prob = model.get_cluster_prob(sentence_rep)

#             if i == 0:
#                 all_labels = label
#                 all_embeddings = sentence_rep.detach()
#                 all_prob = model_prob
#             else:
#                 all_labels = torch.cat((all_labels, label), dim=0)
#                 all_embeddings = torch.cat((all_embeddings, sentence_rep.detach()), dim=0)
#                 all_prob = torch.cat((all_prob, model_prob), dim=0)

#     all_pred = torch.argmax(all_prob, dim=1)
#     all_pred += 1
    
# #     print(all_pred.cpu())
# #     print(all_labels.cpu())
#     print('Pred:', torch.unique(all_pred.cpu(), return_counts = True))
#     print('Label:', torch.unique(all_labels, return_counts = True))
#     acc_model = accuracy_score(all_pred.cpu(), all_labels.cpu())
...
#     pred_labels += 1
#     acc = accuracy_score(pred_labels.cpu(), all_labels.cpu())
...
########## write file ##########
#     with open('log_JSD_model_5.csv', "a") as f:
#         writer = csv.writer(f)
#         time = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
#         contrastive_loss = args.loss['Contrastive_loss'].item()
#         cluster_loss = args.loss['clustering_loss'].item()
        
#         row = [time, step, contrastive_loss, cluster_loss, acc, acc_model, args.note]
        
#         if f.tell() == 0:
#             header = ['Time', 'Step', 'Contrastive Loss', 'Clustering Loss', '[Representation] ACC', '[Model] ACC', 'Note: scale = 1,1']
